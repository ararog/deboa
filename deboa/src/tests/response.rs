use crate::{errors::DeboaError, response::DeboaResponse, tests::utils::url};
...
fn test_set_raw_body() -> Result<(), DeboaError> {
    let mut response = DeboaResponse::new(url(), http::StatusCode::OK, http::HeaderMap::new(), &Vec::new());
...
fn test_raw_body() -> Result<(), DeboaError> {
    let response = DeboaResponse::new(url(), http::StatusCode::OK, http::HeaderMap::new(), SAMPLE_TEST);
    assert_eq!(response.raw_body(), SAMPLE_TEST);
    Ok(())
}
