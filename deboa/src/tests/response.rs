...
fn test_status() -> Result<(), DeboaError> {
    let response = DeboaResponse::new(http::StatusCode::OK, http::HeaderMap::new(), &Vec::new());
...
fn test_set_raw_body() -> Result<(), DeboaError> {
    let mut response = DeboaResponse::new(http::StatusCode::OK, http::HeaderMap::new(), &Vec::new());
...
fn test_raw_body() -> Result<(), DeboaError> {
    let response = DeboaResponse::new(http::StatusCode::OK, http::HeaderMap::new(), SAMPLE_TEST);
    assert_eq!(response.raw_body(), SAMPLE_TEST);
    Ok(())
}
